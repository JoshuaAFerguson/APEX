...
describe('AgentTerminalPanel - Animation Implementation (ADR-0043)', () => {
  ...
  describe('ADR-0043 Compliance: Animation Constants Integration', () => {
    ...
    it('applies correct PANEL_WIDTHS classes for maximized state', () => {
      const { rerender, container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )

      let panel = container.firstChild as HTMLElement
      // Normal state should not have col-span-full
      expect(panel).not.toHaveClass('col-span-full')

      rerender(<AgentTerminalPanel {...defaultProps} panelState="maximized" />)
      panel = container.firstChild as HTMLElement
      expect(panel).toHaveClass('col-span-full') // PANEL_WIDTHS.maximized
    })
  ...
  describe('ADR-0043 Compliance: Content Fade Animation (Normal/Maximized states)', () => {
    beforeEach(() => {
      mockLogs = [
    ...
    it('applies panel-content-animate class to content area', () => {
      const { container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )
    ...
    it('sets data-expanded attribute correctly for content animation', () => {
      const { rerender, container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )

      let contentArea = container.querySelector(`[${PANEL_CONTENT_CLASSES.expandedAttr}]`)
      expect(contentArea).toHaveAttribute(PANEL_CONTENT_CLASSES.expandedAttr, 'true')

      rerender(<AgentTerminalPanel {...defaultProps} panelState="maximized" />)
      contentArea = container.querySelector(`[${PANEL_CONTENT_CLASSES.expandedAttr}]`)
      expect(contentArea).toHaveAttribute(PANEL_CONTENT_CLASSES.expandedAttr, 'true')
    })

    it('applies panel-content-inner class for overflow control', () => {
      const { container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )

      const innerContent = container.querySelector(`.${PANEL_CONTENT_CLASSES.inner}`)
      expect(innerContent).toBeInTheDocument()
    })

    it('content has correct visibility classes based on panel state', () => {
      const { container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )
      ...
      expect(contentArea).toHaveClass('visible')
    })

    it('content has aria-hidden attribute based on panel state', () => {
      const { container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )
  ...
  describe('State Transition Animation Flow', () => {
    ...
    it('preserves content animation state during transitions (only for normal/maximized)', () => {
      const { rerender, container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )
      ...
      // Transition to maximized (should still have content)
      rerender(<AgentTerminalPanel {...defaultProps} panelState="maximized" />)
  ...
  describe('CSS Grid Animation Integration', () => {
    it('content area uses CSS grid for smooth height transitions', () => {
      const { container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )
    ...
    it('inner content area has proper overflow control', () => {
      const { container } = render(
        <AgentTerminalPanel {...defaultProps} panelState="normal" />
      )
      ...
      const innerContent = container.querySelector(`.${PANEL_CONTENT_CLASSES.inner}`)
      expect(innerContent).toBeInTheDocument()
      expect(innerContent).toHaveClass(PANEL_CONTENT_CLASSES.inner)
    })
  })
