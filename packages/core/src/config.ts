...
export function getEffectiveConfig(config: ApexConfig): Required<ApexConfig> {
  return {
    ...
    teams: config.teams ? {
      ...
      tenantId: config.teams.tenantId ?? 'common',
      oauthConnectionName: config.teams.oauthConnectionName,
      defaultTeamId: config.teams.defaultTeamId,
      defaultChannelId: config.teams.defaultChannelId,
      useAdaptiveCards: config.teams.useAdaptiveCards ?? true,
      threadUpdates: config.teams.threadUpdates ?? true,
    ...
    discord: config.discord ? {
      ...
      defaultChannelId: config.discord.defaultChannelId,
      notificationChannelIds: config.discord.notificationChannelIds ?? [],
      threadUpdates: config.discord.threadUpdates ?? true,
      useEmbeds: config.discord.useEmbeds ?? true,
      mode: config.discord.mode ?? 'gateway',
      interactionsEndpoint: config.discord.interactionsEndpoint,
    } : {
      enabled: false,
      botToken: undefined,
      ...
      interactionsEndpoint: undefined,
    },
    webhooks: config.webhooks ? {
      enabled: config.webhooks.enabled ?? true,
      maxWebhooks: config.webhooks.maxWebhooks ?? 50,
      defaultTimeoutMs: config.webhooks.defaultTimeoutMs ?? 30000,
      maxConcurrentDeliveries: config.webhooks.maxConcurrentDeliveries ?? 10,
      logRetentionDays: config.webhooks.logRetentionDays ?? 30,
      requireHttps: config.webhooks.requireHttps ?? true,
      allowLocalhost: config.webhooks.allowLocalhost ?? false,
      defaultEvents: config.webhooks.defaultEvents ?? ['task:completed', 'task:failed', 'approval:required'],
    } : {
      enabled: true,
      maxWebhooks: 50,
      defaultTimeoutMs: 30000,
