 ...
 *
 * Extracted to its own file to avoid circular dependencies: the individual analyzer
 * implementations (maintenance-analyzer.ts, etc.) need to import BaseAnalyzer, and
 * index.ts re-exports those implementations.
 */
...
/**
 * Types of specific remediation actions that can be suggested
 */
...
/**
 * A specific remediation suggestion with actionable steps
 */
export interface RemediationSuggestion {
  ...
  /** Exact command to execute (if applicable) */
  command?: string;

  /** Link to relevant documentation or advisory */
  link?: string;

  /** Priority level for this specific suggestion */
  priority: 'critical' | 'high' | 'medium' | 'low';

  /** Expected outcome after completing this suggestion */
...
export interface TaskCandidate {
   ...
   * Provides concrete commands and guidance for resolving the issue.
   */
...
export abstract class BaseAnalyzer implements StrategyAnalyzer {
  ...
  protected createCandidate(
    ...
    return {
      ...
      remediationSuggestions: options.remediationSuggestions,
    };
  }
}
