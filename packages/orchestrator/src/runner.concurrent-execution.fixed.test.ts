...
/**
 * Fixed test suite for concurrent task execution implementation in DaemonRunner.
...
vi.mock('./daemon-scheduler', () => ({
  DaemonScheduler: vi.fn(() => ({
    ...
    getUsageStats: vi.fn().mockReturnValue({
      ...
      capacity: { threshold: 0.9, currentPercentage: 0.5 },
    }),
  })),
...
vi.mock('./context', () => ({
  createContextSummary: vi.fn().mockReturnValue('Test context summary'),
}));
