...
/**
 * Configuration options for the DaemonRunner.
 * Defines all settings needed to initialize and run the APEX daemon,
 * including project path, polling behavior, concurrency limits, and logging configuration.
 */
...
/**
 * Real-time metrics and status information for the DaemonRunner.
 * Provides comprehensive statistics about daemon performance, task processing,
 * and current operational state for monitoring and debugging purposes.
 */
...
/**
 * Structure for daemon log entries.
 * Defines the format of log messages generated by the DaemonRunner,
 * including timestamp, severity level, message content, and optional metadata.
 */
...
/**
 * APEX Daemon Runner - Autonomous task execution engine
 *
 * The DaemonRunner orchestrates continuous task processing for APEX projects,
 * managing task queues, capacity limits, error recovery, and integrated services.
 * It provides robust task execution with automatic pause/resume, orphan detection,
 * and comprehensive monitoring capabilities.
 *
 * Key features:
 * - Concurrent task execution with configurable limits
 * - Automatic capacity monitoring and pause/resume functionality
 * - Orphaned task detection and recovery
 * - Integrated API server and Web UI management
 ...
 * console.log(`API running: ${api}, WebUI running: ${webui}`);
 * ```
 */
export class DaemonRunner {
   ...
   *
   * Initializes the complete daemon environment including orchestrator,
   * task store, capacity monitoring, integrated services, and polling.
   * Sets up signal handlers for graceful shutdown and begins task processing.
   *
   * @throws {Error} If daemon is already running or initialization fails
   * @returns Promise that resolves when daemon startup is complete
   *
   * @example
   * ```typescript
   * const daemon = new DaemonRunner({
   ...
   *
   * Initiates graceful shutdown by stopping polling, waiting for running tasks
   * to complete (with timeout), and cleaning up all resources including
   * integrated services, capacity monitoring, and file handles.
   *
   * @param timeout - Maximum time to wait for tasks to complete (default: 30s)
   ...
   * @example
   * ```typescript
   * // Stop with monitoring of running tasks
   * const metrics = daemon.getMetrics();
   ...
   *
   * Returns comprehensive real-time statistics about daemon performance,
   * task processing status, and operational state. Useful for monitoring,
   * debugging, and capacity planning.
   *
   * @returns Current daemon metrics including uptime, task counts, and status
   *
   * @example
   * ```typescript
   * const metrics = daemon.getMetrics();
   ...
   * console.log(`  Tasks processed: ${metrics.tasksProcessed}`);
   * console.log(`  Success rate: ${(metrics.tasksSucceeded / metrics.tasksProcessed * 100).toFixed(1)}%`);
   * console.log(`  Active tasks: ${metrics.activeTaskCount}/${maxConcurrentTasks}`);
   *
   * if (metrics.isPaused) {
   *   console.log(`  Paused: ${metrics.pauseReason}`);
   * }
   * ```
   *
   * @example
   * ```typescript
   * // Monitor daemon health periodically
   * setInterval(() => {
   *   const metrics = daemon.getMetrics();
